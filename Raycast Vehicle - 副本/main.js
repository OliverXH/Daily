...
function initObject() {
    ...
    objLoader.load("assert/car.obj", createCar);
    function createCar(obj) {
        ...
        // FL
        options.chassisConnectionPointLocal.set(-0.79, 1.3, 0.2); // 设置锚点位置
        vehicle.addWheel(options);

        // FR
        options.chassisConnectionPointLocal.set(0.79, 1.3, 0.2);
        vehicle.addWheel(options);

        // RL
        options.chassisConnectionPointLocal.set(-0.79, -1.126, 0.2);
        vehicle.addWheel(options);
    ...
    function render() {
        ...
        if (chassis.mesh) {
            ...
            for (let i = 0; i < wheelMeshes.length; i++) {
                ...
                meshes[i].position.copy(wheelBodies[i].position);
                meshes[i].quaternion.copy(wheelBodies[i].quaternion);
            }
        }
