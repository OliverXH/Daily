...
let PhysicsEngine = (function () {
    ...
    */
    ...
    PhysicsEngine.prototype._addMeshVerts = function (btTriangleMesh, topLevelObject, object) {
        let geometry = new THREE.Geometry().fromBufferGeometry(mesh.geometry);
        ...
        for (let i = 0; i < geometry.faces.length; i++) {
            ...
            let a = scale.x * geometry.vertices[geometry.faces[i].a].x;
            let b = scale.y * geometry.vertices[geometry.faces[i].a].y;
            let c = scale.z * geometry.vertices[geometry.faces[i].a].z;

            vertex.setValue(a, b, c);
            triPoints.push(vertex);

            a = scale.x * geometry.vertices[geometry.faces[i].b].x;
            b = scale.y * geometry.vertices[geometry.faces[i].b].y;
            c = scale.z * geometry.vertices[geometry.faces[i].b].z;

            vertex.setValue(a, b, c);
            triPoints.push(vertex);

            a = scale.x * geometry.vertices[geometry.faces[i].c].x;
            b = scale.y * geometry.vertices[geometry.faces[i].c].y;
            c = scale.z * geometry.vertices[geometry.faces[i].c].z;
        ...
        object.children.forEach(function (child) {
            triangleCount += this._addMeshVerts(btTriangleMesh, topLevelObject, child);
        });
    ...
    */
            ...
            if (object.geometry.parameters !== void 0) {
                switch (object.geometry.type) {
                    ...
                    case 'TriMesh':
                        ...
                        let tetraMesh = new Ammo.btTriangleMesh();

                        let triangleCount = this._addMeshVerts(tetraMesh, object, object);
                        if (triangleCount == 0) {
                            returnShape = new Ammo.btCompoundShape();
                        }
                        else {
                            returnShape = new Ammo.btBvhTriangleMeshShape(tetraMesh);
                        }
                        break;
                }
            } else {
     ...
     */
    PhysicsEngine.prototype.generateRigidBody = function (object, type, params) {
        ...
        if (object.type == 'Mesh') {
            colShape = this._createShape(object, false);
        } else {
                ...
                if (child.hasOwnProperty('userData')) {
                    ...
                    if (child.userData.hasOwnProperty('data')) {
                        if (child.userData.data == 'collision' || child.userData.data == 'physics') {
                            ...
                            } else if (child.userData.shape == 'trimesh') {

                                child.geometry.type = 'TriMesh';
            ...
            colShape = this._createShape(collisionShape, false);
        }
    ...
    PhysicsEngine.prototype.makeConstraintTransforms = function (transform1, transform2, body1, body2, pivot, orn) {

        let pivot_transform = new Ammo.btTransform();
        pivot_transform.setIdentity();
        pivot_transform.setOrigin(new Ammo.btVector3(pivot.x, pivot.y, pivot.z));
        pivot_transform.setRotation(new Ammo.btQuaternion(orn.x, orn.y, orn.z, orn.w));

        let tmpTrans1 = body1.getWorldTransform();
        let tmpTrans2 = body2.getWorldTransform();

        transform1 = tmpTrans1.op_mul(pivot_transform);
        transform2 = tmpTrans2.op_mul(pivot_transform);

        console.log(pivot_transform.getOrigin().y(), transform1.getOrigin().y());

    }

    PhysicsEngine.prototype.addConstraint = function (type, body1, body2, params) {

        let pivot = params.pivot || new THREE.Vector3();
        let orn = params.orn || new THREE.Quaternion();
        ...
        switch (type) {
            case 'Fixed':
                // transform1 = new Ammo.btTransform();
                // transform2 = new Ammo.btTransform();

                this.makeConstraintTransforms(transform1, transform2, body1, body2, pivot, orn);

                constraint = new Ammo.btFixedConstraint(body1, body2, transform1, transform2);
                break;

            case 'Point':
                let pivot1 = body1.getWorldTransform().inverse() * pivot;
                let pivot2 = body2.getWorldTransform().inverse() * pivot;

                constraint = new Ammo.btPoint2PointConstraint(body1, body2, pivot1, pivot2);
                break;

            case 'Hinge':
                // if (!params.mainAxis) {
                //     params.mainAxis = new Ammo.btVector3(0, 0, 0);
                // }
                // if (!params.connectedAxis) {
                //     params.connectedAxis = new Ammo.btVector3(0, 0, 0);
                // }
                ...
                constraint = new Ammo.btHingeConstraint(body1, body2, transform1, transform2);
                break;
            ...
            case 'Slider':
                ...
                constraint = new Ammo.btSliderConstraint(body1, body2, transform1, transform2, true);
                break;
            ...
            case 'Piston':
                ...
                constraint.setUpperAngLimit(-1.0);  // unlock rotation axis
                break;
        ...
        console.log(body1.getWorldTransform().getOrigin().x(), transform2.getOrigin().x());
        console.log(transform1.getOrigin().y(), transform2.getOrigin().y());
        console.log(transform1.getOrigin().z(), transform2.getOrigin().z());

        // this.world.addConstraint(constraint, params.collision);
